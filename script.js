var P2 = (function () {
  ...
  var createSecretCode = function () {
    ...
    for (let i = 0; i < codeLength; i++) {
      ...
      secretCodeMessage += code[i]["name"] + " ";
    }
    console.log(secretCodeMessage);
  };
  ...
  var buildTableHeader = function () {
    ...
    for (var index in keys) {
      ...
      headerCol.innerHTML = keys[index];
      if (index == 0 || index == 2) {
        headerCol.rowSpan = 2;
      } else {
        headerCol.colSpan = 4;
    ...
    for (index in numberKeys) {
      var numberCol = document.createElement("th");
      numberCol.innerHTML = numberKeys[index];
      numberHeader.appendChild(numberCol);
  ...
  var buildTableRow = function (result) {
    ...
    if (isGameOver) {
      ...
      messageOutput.innerHTML +=
        " Congratulations!  You guessed the code in only " +
        guesses +
        " guess(es)!";
  ...
  var isGuessValid = function () {
    ...
    if (!valid) {
      alert("Please select a color for ALL 4 boxes!");
    }
